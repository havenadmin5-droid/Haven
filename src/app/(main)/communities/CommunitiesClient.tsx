...
/**
 * Communities skeleton for instant loading state
 */
...
export function CommunitiesClient() {
  ...
  const { data: fetchedCommunities, isLoading: communitiesLoading } = useCommunities();
  ...
  const filteredCommunities = useMemo(() => {
    return communities.filter((community) => {
      ...
      if (showMyCommunitiesOnly && !community.is_member) {
        return false;
      }
      ...
      // Tag filter
      if (tagFilter !== "All" && community.tag !== tagFilter) {
        return false;
      }
    ...
    <div>
      ...
      {filteredCommunities.length > 0 ? (
        ...
        <div className="card text-center py-12">
          ...
          {showMyCommunitiesOnly && (
            <button
              onClick={() => setShowMyCommunitiesOnly(false)}

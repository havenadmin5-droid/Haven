...
import { useAuth, useDirectoryProfiles } from "@/hooks";
import type { PublicProfile } from "@/lib/types";
import { useRouter } from "next/navigation";

/**
 * Directory skeleton for instant loading state
 */
...
export function DirectoryClient() {
  ...
  const filteredProfiles = useMemo(() => {
    ...
    return profiles.filter((profile) => {
      ...
      // City filter
      if (cityFilter !== "All" && profile.display_city !== cityFilter) {
        return false;
      }

      // Profession filter
      if (professionFilter !== "All" && profile.profession !== professionFilter) {
        return false;
      }
      ...
      <div className="card mb-6">
        ...
        {(cityFilter !== "All" || professionFilter !== "All" || debouncedSearch) && (
          <div className="flex items-center gap-2 mt-4 pt-4 border-t border-[var(--border-color)]">
            ...
            {cityFilter !== "All" && (
              <span className="px-2 py-1 rounded-full text-xs bg-[var(--teal)] text-white">
                {cityFilter}
              </span>
            )}
            {professionFilter !== "All" && (
              <span className="px-2 py-1 rounded-full text-xs bg-[var(--amber)] text-white">
                {professionFilter}
              </span>
            )}
            <button
